...
> Two implementation notes that cost debugging time, recorded for the next
> person: CoreMediaIO delivers device arrivals through **main-run-loop
> callbacks**, so discovery code that sleeps instead of pumping the run loop
> sees zero devices forever (indistinguishable from an unsupported phone);
> and on current macOS the capture device's `uniqueID` is a freshly minted
> UUID rather than the phone's UDID, so sim-use matches by the device **name**
> devicectl reports, falling back to a sole connected iOS device.
