...
| Real device: `record-video` says the device is not visible as a screen-capture device | Screen recording rides CoreMediaIO, which only sees **USB**-connected phones — Wi-Fi is not enough | Plug the cable in (and keep the Mac trusted). A first run may also raise a one-time macOS camera-permission prompt for the terminal — ask the user to approve it |
