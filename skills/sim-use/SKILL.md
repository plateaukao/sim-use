...
| Real device: `record-video` says the device is not visible as a screen-capture device | It is not on USB (CoreMediaIO cannot see Wi-Fi phones), or another app (QuickTime, OBS) is already capturing it | Ask the user to plug in the cable, approve the one-time camera prompt, and close any app that is recording the phone |
